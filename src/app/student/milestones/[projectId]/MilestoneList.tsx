...
import { updateMilestone } from '@/src/server/data/milestone'
...
interface MilestoneListProps {
  ...
  onMilestoneUpdate: () => void // Add this prop to refresh data
}

export function MilestoneList({ milestones, onMilestoneUpdate }: MilestoneListProps) {
  ...
  const handleSave = async (milestone: ExtendedMilestone) => {
    ...
    try {
      ...
      onMilestoneUpdate() // Refresh the milestones data
    } catch (error) {
    ...
    <div>
      ...
      {milestones.length === 0 ? (
        ...
        milestones.map((milestone) => (
          <div key={milestone.id} className='mb-4 rounded-md border p-4'>
